...
## Original particle atlas, 2026-09-19

`particle-atlas.webp` is original procedural artwork generated by
`tools/author-particles.py` using NumPy 2.2.5 and Pillow 12.1.0 (lossless WebP).
The 1024×1024 linear RGB coverage mask has sixteen 256×256 cells, read left to
right and top to bottom: four fbm puffs, four hot-core embers, four horizontal
streaks, two droplets and two six-armed snowflakes. Black gutters prevent edge
bleed; coverage is sampled from red, not alpha. No photography, downloaded
models, stock imagery or film frames are included. NumPy's fixed seed is
20260916; regeneration requires only Pillow and NumPy. The checked-in image is
97,906 bytes; its checksum is recorded in `tools/asset-baseline.json`.
