...
const cinema=createCinema({renderer,scene,camera,canvas,sun,buildings,ground,ship,hullMat,core,beam,blast,ocean,wave,meteor,planet,landscape,windows,snow,debris,foam,clouds,glow});
const production=await createProduction({renderer,scene,camera,canvas,city,buildings,ground,ship,core,tower,blast,wave,foam,ocean,planet,landscape,sun,beam,meteor,tail});
const scenes=[
 {title:'A close<br>encounter.',description:"They didn't come in peace.",film:'INDEPENDENCE DAY',year:'1996',type:'EXTRATERRESTRIAL EVENT',location:'NEW YORK CITY',color:'#ff6138',sky:'#171f22',phases:['SIGNAL DETECTED','WEAPON CHARGING','CITY-WIDE SHOCKWAVE'],labels:['ARRIVAL','CONTACT','AFTERMATH']},
 {title:'One last<br>wave.',description:'No higher ground. No second chances.',film:'DEEP IMPACT',year:'1998',type:'EXTINCTION-LEVEL IMPACT',location:'ATLANTIC COAST',color:'#ff9852',sky:'#34333a',phases:['OBJECT APPROACHING','OCEAN IMPACT','WAVE INBOUND'],labels:['APPROACH','IMPACT','INUNDATION']},
 {title:'A world<br>on ice.',description:'The forecast is the end of everything.',film:'THE DAY AFTER TOMORROW',year:'2004',type:'GLOBAL SUPERSTORM',location:'MANHATTAN',color:'#8dcfff',sky:'#344555',phases:['PRESSURE FALLING','SUPERSTORM ARRIVES','DEEP FREEZE'],labels:['WARNING','WHITEOUT','DEEP FREEZE']},
 {title:'Nothing<br>will remain.',description:'A beautiful day for the end of the world.',film:'MELANCHOLIA',year:'2011',type:'PLANETARY COLLISION',location:'THE LAST EVENING',color:'#9bcfd9',sky:'#172a35',phases:['ORBIT DECAYING','CLOSER THAN THE MOON','FINAL CONTACT'],labels:['DISCOVERY','APPROACH','COLLISION']}
];
...
function resetCamera(){if(selected===3){camera.position.set(85,31,130);controls.target.set(-8,39,-45);}else{camera.position.set(122,58,155);controls.target.set(-8,35,-15);}controls.update();}
...
function selectScene(index){if(!Number.isInteger(index)||index<0||index>=scenes.length)throw new Error('Scene must be 0, 1, 2, or 3.');selected=index;time=0;playing=false;const s=scenes[index];$('scene-title').innerHTML=s.title;$('scene-description').textContent=s.description;$('film-title').textContent=s.film;$('film-year').textContent=s.year;$('scenario-type').textContent=s.type;$('scene-location').textContent=s.location;$('scene-number').textContent=String(index+1).padStart(2,'0');document.documentElement.style.setProperty('--accent',s.color);document.querySelectorAll('.timeline-ticks span').forEach((el,i)=>el.textContent=s.labels[i]);document.querySelectorAll('.scene-card').forEach((el,i)=>{el.classList.toggle('active',i===index);el.setAttribute('aria-pressed',String(i===index));});canvas.setAttribute('aria-label',`Interactive 3D scene inspired by ${s.film}. Drag to orbit, scroll to zoom, or use arrow keys to orbit.`);resetCamera();updateWorld();syncUI();}
function updateWorld(){needsRender=true;const t=time,s=scenes[selected],hit=ease((t-13)/10);scene.background.set(s.sky);scene.fog.color.set(s.sky);scene.fog.density=selected===2?.004+hit*.004:.0055;sun.color.set(selected===2?'#b4d9fa':selected===3?'#a8d9f0':'#ffe2b7');sun.intensity=selected===2?2-hit:3;
 city.visible=selected!==3;landscape.visible=selected===3;ship.visible=selected===0;beam.visible=selected===0&&t>9&&t<20;core.scale.setScalar(1+ease(t/13)*1.3);ship.position.y=83+(1-ease(t/10))*9;ship.rotation.y=t*.018;beam.material.opacity=clamp((t-9)/3)*clamp((20-t)/3)*.8;beam.scale.x=beam.scale.z=.3+ease((t-9)/4)*2;glow.color.set(selected===0?'#a7ffd1':'#ff9245');glow.intensity=selected<2?Math.sin(clamp((t-10)/13)*Math.PI)*180:0;
...
document.querySelectorAll('[data-scene]').forEach(b=>b.addEventListener('click',()=>selectScene(Number(b.dataset.scene))));$('blast').addEventListener('click',start);$('replay').addEventListener('click',start);$('play').addEventListener('click',()=>{if(time>=30)time=0;playing=!playing;syncUI();});$('speed').addEventListener('click',()=>{speed=speed===1?.5:speed===.5?2:1;$('speed').textContent=speed+'×';});$('progress').addEventListener('input',e=>{time=Number(e.target.value);playing=false;updateWorld();syncUI();});$('reset-camera').addEventListener('click',resetCamera);$('fullscreen').addEventListener('click',async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await document.querySelector('.stage').requestFullscreen();}catch{$('fullscreen').title='Fullscreen is unavailable in this browser';}});
